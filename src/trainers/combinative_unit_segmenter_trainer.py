...
import classifiers.combinative_sequence_tagger
import common
import constants
from data_loaders import segmentation_data_loader
import dictionary
from evaluators.combinative_unit_segmenter_evaluator import (
    AccuracyEvaluatorForAttention, CombinativeSegmenterEvaluator,
    CombinativeTaggerEvaluator)
...
# character word cc combinative segmenter/tagger
class CombinativeUnitSegmenterTrainer(TaggerTrainerBase):
    ...
    def init_model(self):
        super().init_model()
        if self.unigram_embed_model or self.bigram_embed_model or self.chunk_embed_model or self.cc_embed_model:
        ...
        with open(hparams_path, 'r') as f:
            for line in f:
                ...
                if (key == 'pretrained_unigram_embed_size'
                        ...
                        == 'cc_freq_threshold' or key == 'chunk_max_vocab_size'
                    ...
                    val = float(val)
                      ...
                      or key == 'use_gold_chunk' or key == 'use_gold_cc'
                      or key == 'unuse_nongold_chunk'
                      or key == 'unuse_nongold_cc'
    ...
    def setup_classifier(self):
        ...
        self.classifier = classifiers.combinative_sequence_tagger.CombinativeSequenceTagger(
    ...
    def gen_inputs(self, data, ids, evaluate=True, restrict_memory=False):
        ...
        inputs[6]: cc sequences
        inputs[7]: cc additional feature sequences
        inputs[8]: cc masking sequences
        outputs[0]: gold segmentation sequences (label)
        outputs[1]: gold chunk sequences (word)
        outputs[2]: gold cc sequences
        Reverse mode:
            inputs
                3 <-> 6
                4 <-> 7
        ...
        use_cc_attention = (self.hparams['cc_pooling_type'] == constants.CCAVG
                            or self.hparams['cc_pooling_type']
                            == constants.CCCON)
        ...
        qs = [
            segmentation_data_loader.convert_mask_matrix(
                ...
                n_tokens=len(us[i]),
                n_chunks=len(ccs[i]) if ccs else 0,
                feat_size=feat_size,
                emb_size=embed_size,
                use_attention=use_cc_attention,
                device=device) for i, j in enumerate(ids)
        ]
        ...
        if evaluate:
            if self.args.evaluation_method == 'attention':
                ...
                if not self.hparams['reverse']:
                    return us, cs, ds, ccs, cds, ms, qs, bs, fs, gls, gcs, gccs, ncands, ncccands
                else:
                    return us, ccs, cds, cs, ds, qs, ms, bs, fs, gls, gccs, gcs, ncccands, ncands
            else:
                if not self.hparams['reverse']:
                    return us, cs, ds, ccs, cds, ms, qs, bs, fs, gls, gcs, gccs
                else:
                    return us, ccs, cds, cs, ds, qs, ms, bs, fs, gls, gccs, gcs
        else:
            if not self.hparams['reverse']:
                return us, cs, ds, ccs, cds, ms, qs, bs, fs
            else:
                return us, ccs, cds, cs, ds, qs, ms, bs, fs
