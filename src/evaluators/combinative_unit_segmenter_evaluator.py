...
from evaluators.common import Counts, FMeasureAndAccuraciesCalculator, DoubleFMeasureEvaluator
from tools import conlleval


class CombinativeSegmenterEvaluator(object):
    def __init__(self, id2label):
        self.calculator = FMeasureAndAccuraciesCalculator(id2label)
    ...
    def report_results(self, sen_counter, counts, loss, file=sys.stderr):
        ...
        l3_acc = 1. * counts.l3.correct / counts.l3.total if counts.l3.total > 0 else 0
        ...
        print('TP, FP, FN: %d %d %d' % (l1_met.tp, l1_met.fp, l1_met.fn),
              file=file)
        print('A, P, R, F, AW, ACC:%6.2f %6.2f %6.2f %6.2f %6.2f %6.2f' %
              (100. * l1_met.acc, 100. * l1_met.prec, 100. * l1_met.rec,
               100. * l1_met.fscore, (100. * l2_acc), (100. * l3_acc)),
              file=file)

        res = '%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.4f' % (
            (100. * l1_met.acc), (100. * l1_met.prec), (100. * l1_met.rec),
            (100. * l1_met.fscore), (100. * l2_acc), (100. * l3_acc), ave_loss)

        return res


class CombinativeTaggerEvaluator(DoubleFMeasureEvaluator):  # tmp
    def __init__(self, id2label):
